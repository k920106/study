...
class JpaWalletRepositoryTest(
    ...
    fun `should update the balance of wallet successfully when execute the updated command at the same time`() {
        val jpaWalletEntity1 = JpaWalletEntity(
            userId = 1, balance = BigDecimal.ZERO, version = 0
        )

        val jpaWalletEntity2 = JpaWalletEntity(
            userId = 2, balance = BigDecimal.ZERO, version = 0
        )
        ...
        Assertions.assertThat(retrievedWallet1.version).isEqualTo(3)
        Assertions.assertThat(retrievedWallet2.version).isEqualTo(3)

        Assertions.assertThat(retrievedWallet1.balance.toInt()).isEqualTo(6000)
        Assertions.assertThat(retrievedWallet2.balance.toInt()).isEqualTo(6000)
    }
}
