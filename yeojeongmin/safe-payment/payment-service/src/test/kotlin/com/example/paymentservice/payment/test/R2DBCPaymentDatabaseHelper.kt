...
class R2DBCPaymentDatabaseHelper(
    ...
    override fun getPayments(orderId: String): PaymentEvent? {
        return databaseClient.sql(SELECT_PAYMENT_QUERY)
                             ...
                             .flatMap { groupedFlux -> groupedFlux.collectList().map {
                               results -> PaymentEvent(id = groupedFlux.key(),
                                                       ...
                                                       paymentOrders = results.map { result -> PaymentOrder(id = result["id"] as Long,
    ...
    override fun clean(): Mono<Void> {
        return deletePaymentOrderHistories().flatMap { deletePaymentOrders() }
